...
def test_plans_one_request_per_month(tmp_path):
...
def test_full_set_is_840_chunks(tmp_path):
...
def test_downloads_and_then_resumes(tmp_path):
    client = FakeClient()
    first = era5_daily.download([2020], ["t2m_max"], client=client, base_dir=tmp_path)
    ...
    again = FakeClient()
    second = era5_daily.download([2020], ["t2m_max"], client=again, base_dir=tmp_path)
    assert len(second) == 12
    assert again.calls == []

    forced = FakeClient()
    era5_daily.download([2020], ["t2m_max"], client=forced, base_dir=tmp_path, force=True)
